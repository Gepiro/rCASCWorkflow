seuratPermutationSW <- function(group=c("sudo","docker"), scratch.folder, file, percent, separator, logTen=0,pcaDimensions,seed=1111,sparse=FALSE,format="NULL",resolution=0.6, index=1){
...
system(paste("cp -r ",data.folder,"/",matrixName,"/ ",scrat_tmp.folder,"/",sep=""))
  ...
  #executing the docker job
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",scrat_tmp.folder,":/scratch -v ", data.folder, ":/data -d docker.io/repbioinfo/seuratpermutationsw Rscript /home/main.R ",matrixName," ",percent," ",format," ",separator," ",logTen," ",pcaDimensions," ",seed," ",sparse," ",resolution, " ", index, sep="")
